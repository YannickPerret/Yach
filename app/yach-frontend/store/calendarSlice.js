...
export const loadTemporaryCalendar = createAsyncThunk(
  ...
  async ({ calendarURL, calendarName }) => {
    const response = await fetch(`http://localhost:3000/api/v1/calendar/temporary`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
export const updateUserCalendarAsync = createAsyncThunk(
  ...
  async (data) => {
    const response = await fetch(`http://localhost:3000/users/${data.username}/calendars/${data.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json'
      },
...
export const updateCalendarEventAsync = createAsyncThunk(
  ...
  async (data) => {
    await fetch(`http://localhost:3000/users/${data.username}/calendars/${data.calendarId}/events`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json'
      },
...
export const calendarSlice = createSlice({
  ...
  reducers: {
    ...
    removeUserCalendar: (state, action) => {
      state.userCalendars = state.userCalendars.filter(calendar => calendar.id !== action.payload.id);
    },
    updateUserCalendar: (state, action) => {
      const index = state.userCalendars.findIndex(calendar => calendar.id === action.payload.id);
      if (index !== -1) {
        state.userCalendars[index] = action.payload;
      }
    },
    updateCalendarEvents: (state, action) => {
      const calendarToUpdate = state.userCalendars.find(calendar => calendar.id === action.payload.calendarId);
      if (calendarToUpdate) {
    ...
    removeTemporaryCalendar: (state, action) => {
      state.temporaryCalendars = state.temporaryCalendars.filter(calendar => calendar.id !== action.payload);
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateCalendarEventAsync.fulfilled, (state, action) => {
        ...
        const updatedCalendar = state.userCalendars.find(calendar => calendar.id === action.payload.calendarId);
        if (updatedCalendar) {
...
export const selectTemporaryCalendars = state => state.calendars.temporaryCalendars;
export const selectSelectedCalendar = state => state.calendars.selectedCalendar;
