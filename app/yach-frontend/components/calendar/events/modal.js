...
import { selectSelectedCalendar, updateUserCalendar } from '@/store/calendarSlice';


export default function ModalEvent({ event, onClose, isEdit = false, canEditing, onEventChange }) {
    ...
    const [eventData, setEventData] = useState({
        ...
        summary: event?.title || '',
        startDate: event && event.start ? formatDateToDatetimeLocal(event.start) : formatDateToDatetimeLocal(new Date()),
        endDate: event && event.end ? formatDateToDatetimeLocal(event.end) : formatDateToDatetimeLocal(new Date(Date.now() + 3600000)),  // 3600000ms = 1 hour

        interval: event && event.interval !== undefined && event.interval >= 1 ? event.interval : 1,
        description: event ? event.description : '',
        location: event ? event.location : '',
        recurrenceType: event?.recurrenceType || '',
        rRule: event?.rRule ? event.recurringDef.typeData.rruleSet._rrule[0].toText() : "null",
        selectedCalendar: [],
    });    
    ...
    const handleSelectChange = (e) => {
        const selectedCalendarIds = Array.from(e.target.selectedOptions).map(option => option.value);
        setEventData(prevState => ({ ...prevState, selectedCalendar: selectedCalendarIds }));
    };    
    ...
    const handleSubmitEvent = async (e) => {
        ...
        if (doesEventOverlap(eventData.startDate, eventData.endDate)) {
            dispatch(setErrorMessage("Un événement existe déjà pour cette plage horaire!"));
            return;
        }
        ...
        try {
            await fetch(`http://localhost:3000/api/v1/calendar/${eventData.calendarId}`, {
                method: 'PUT',
                headers: {
    ...
    return (
        <Modal onClose={onClose}>
            ...
            <form id="eventForm" className="eventFormModal" onSubmit={handleSubmitEvent}>
                ...
                <input type="text" value={eventData.rRule} />
            ...
            {calendar?.type === 'SHARED' &&
                <div>
                    ...
                    <select id="eventCalendarSelection" name="eventCalendarSelection" multiple onChange={handleSelectChange} disabled={!canEditing}>
                        ...
                        {calendar.children.map(childCalendar => (
