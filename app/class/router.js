module.exports = (app, handlers, upload) => {
  app.register(
    (instance, opts, next) => {
      ...
      instance.post(
        ...
        handlers.submitCalendar
      );

      instance.delete("calendar/:calendarId", handlers.removeCalendar);
  ...
  app.get(
    "/users/:id/calendars/:calendarId/events",
    handlers.getUserCalendarEvents
  ); // voir les événements d'un calendrier de l'utilisateur // format liste
  app.put(
    "/users/:id/calendars/:calendarId/events",
    handlers.updateUserCalendarEvents
  ); // modifier les événements d'un calendrier de l'utilisateur
  app.get(
    "/users/:id/calendars/:calendarId/events/:eventId",
    handlers.getUserCalendarEventById
  ); // voir un événement d'un calendrier de l'utilisateur
  app.post(
    "/users/:id/calendars/:calendarId/import",
    { preHandler: upload.single("file") },
    handlers.importUserCalendar
  ); // importer un calendrier pour l'utilisateur
  app.post(
    "/users/:id/calendars/:calendarId/subscribe",
    handlers.subscribeUserCalendar
  ); // s'abonner à un calendrier
  app.post(
    "/users/:id/calendars/:calendarId/unsubscribe",
    handlers.unsubscribeUserCalendar
  ); // se désabonner à un calendrier
