...
class Calendar {
  ...
  updateChildrenCalendars = async (calendarUpdated) => {
    ...
    if (
      currentChildIds.sort().toString() !== updatedChildIds.sort().toString()
    ) {
  ...
  updateTaskScheduler = async (calendarUpdated) => {
    ...
    if (currentCron !== updatedCron) {
      ...
      taskSchedulerManager.updateTask(
        `Sync ${this.name}`,
        updatedCron,
        this.sync
      );
    } else {
      taskSchedulerManager.addTask(updatedCron, this.sync, {
        name: `Sync ${this.name}`,
      });
    }
  };
  ...
  async getChildCalendars() {
    const childCalendars =
      await Database.getInstance().CalendarAssociation.findMany({
        where: {
          parentCalendarId: this.id,
        },
        include: {
    ...
    return childCalendars
      ? childCalendars.map(
          (childCalendar) => new Calendar(childCalendar.childCalendar)
        )
      : [];
  }
  ...
  async addEvent(event) {
    try {
      ...
      console.error(
        `Error adding event to calendar with id: ${this.id}`,
        error
      );
      throw error;
    }
  ...
  async addEventWithoutAssociation(event) {
    try {
      await event.persist();
      await this.#associateEventWithCalendar(event.id);
    } catch (error) {
  ...
  async getEvents() {
    try {
      const events =
        await Database.getInstance().calendarEventAssociation.findMany({
          where: {
            calendarId: this.id,
          },
          include: {
  ...
  mergeEventsCalendar = async (eventsToMerge) => {
    ...
    for (const newEvent of eventsToMerge) {
      ...
      if (
        !matchingEvent ||
        JSON.stringify(newEvent) !== JSON.stringify(matchingEvent)
      ) {
  ...
  static async getById(id) {
    try {
      ...
      const events = calendarEventAssociations.map(
        (association) => new Event(association.event)
      );
  ...
  static async getChildCalendarsWithEvents(parentCalendar, childAssociations) {
    for (const association of childAssociations) {
      const childCalendarData =
        await Database.getInstance().calendar.findUnique({
      ...
      const {
        ...
        childCalendars,
        calendarUsersAssociations,
        ...otherChildCalendarData
      } = childCalendarData;
      const childEvents = calendarEventAssociations.map(
        (association) => new Event(association.event)
      );
      const childUsers = calendarUsersAssociations.map(
        (association) => association.user
      );

      const childCalendar = new Calendar({
        ...otherChildCalendarData,
        events: childEvents,
        users: childUsers,
      });
  ...
  async getParentCalendars() {
    ...
    return parentCalendars
      ? parentCalendars.map((association) => association.parentCalendar)
      : [];
  }
  ...
  static async getAll(filter = null) {
    try {
      ...
      let calendars = calendarsData.map(
  ...
  async remove() {
    try {
      ...
      const eventIds = await Database.getInstance()
        .CalendarEventAssociation.findMany({
          where: { calendarId: this.id },
          select: { eventId: true },
        })
        .then((events) => events.map((e) => e.eventId));

      // Suppression des associations d'événements
      if (eventIds.length > 0) {
        console.log("Number of events to delete:", eventIds.length);
      ...
      // Trouver si ce calendrier a des calendriers enfants, puis supprimer les associations
      const childCalendars =
        await Database.getInstance().CalendarAssociation.findMany({
      ...
      if (childCalendars.length > 0) {
        console.log(
          "Number of child calendar associations to delete:",
          childCalendars.length
        );
      ...
      // Trouver si ce calendrier est un enfant d'un autre calendrier, puis supprimer les associations
      const parentAssociations =
        await Database.getInstance().CalendarAssociation.findMany({
          where: { childCalendarId: this.id },
        });

      if (parentAssociations.length > 0) {
        console.log(
          "Number of parent calendars to dissociate from:",
          parentAssociations.length
        );
        await Database.getInstance().CalendarAssociation.deleteMany({
          where: { childCalendarId: this.id },
        });
      ...
      // Trouver si ce calendrier a des utilisateurs, puis supprimer les associations
      const calendarUsers =
        await Database.getInstance().CalendarUserAssociation.findMany({
          where: { calendarId: this.id },
        });

      if (calendarUsers.length > 0) {
        console.log(
          "Number of calendar users to delete:",
      ...
      // Finalement, supprimer le calendrier lui-même
      await Database.getInstance().Calendar.delete({
        where: { id: this.id },
      });
    } catch (error) {
  ...
  addToTaskScheduler() {
    const taskSchedulerManager = TaskScheduler.getInstance();
    taskSchedulerManager.addTask(this.syncExpressionCron, this.sync, {
      name: `Sync ${this.name}`,
    });
  }
  ...
  async #associateUserWithCalendar(userId) {
    const associationExists =
      await Database.getInstance().CalendarUserAssociation.findUnique({
        where: {
          userId_calendarId: {
            userId: userId,
            calendarId: this.id,
