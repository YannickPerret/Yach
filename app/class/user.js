...
class User {
  ...
  async haveAccessToCalendar(calendarId) {
    try {
      ...
      console.error(
        "Erreur lors de la vérification de l'accès au calendrier:",
        error
      );
      return false;
    }
  ...
  async removeSubscribeCalendar(calendarId) {
    ...
    const association =
      await Database.getInstance().CalendarUserAssociation.delete({
        where: {
          userId_calendarId: {
            calendarId: calendarId,
            userId: this.id,
  ...
  async fetchCalendarWithEvents(calendar) {
    try {
      ...
      for (let subCalendarAssoc of subCalendars) {
        const subCalendar = await this.fetchCalendarWithEvents(
          subCalendarAssoc.childCalendar
        );
