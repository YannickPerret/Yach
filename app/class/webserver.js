...
class Webserver {
  ...
  getTemporaryCalendar = async (req, reply) => {
    ...
    let { url: urlCalendarTemporary, name: calendarTemporaryName } = req.body;

    if (!urlCalendarTemporary) {
      return reply.status(400).send({ error: "No url provided" });
    }

    calendarTemporaryName ? calendarTemporaryName : "Yoda Temporary";

    let TemporaryCalendar = new Calendar({
      source: urlCalendarTemporary,
      type: "OTHER",
      name: calendarTemporaryName,
      class: "PUBLIC",
      right: "WRITE",
      url: urlCalendarTemporary,
    });
  ...
  submitCalendar = async (req, reply) => {
    ...
    const {
      file: data,
      body: {
        calendars: inputCalendarsSelected,
        calendarUrl: inputCalendarUrl,
        classification: classification,
        name: nameCalendarInput,
        class: class_visiblity,
        username,
      },
    } = req;

    const classificationCalendar = [
      "PROFESSIONAL",
      "PERSONAL",
      "OTHER",
      ...
      ? classification
      : "OTHER";
    const classVisibility = ["PUBLIC", "PRIVATE"].includes(class_visiblity)
      ? class_visiblity
      : "PUBLIC";
    ...
    if (data && inputCalendarUrl) {
      return reply.status(400).send({
        error:
          "You can't upload a file and add a calendar url at the same time",
      });
    }

    const selectedCalendars =
      inputCalendarsSelected && Array.isArray(inputCalendarsSelected)
        ? inputCalendarsSelected
        : [];

    if (data) {
      console.log(data);
      outputCalendar = await this.handleFileUpload(
        data,
    ...
    if (inputCalendarUrl) {
      outputCalendar = await this.handleUrlCalendar(
        inputCalendarUrl,
        classificationCalendar,
        nameCalendar,
        classVisibility,
    ...
    if (selectedCalendars.length > 1) {
      outputCalendar = await this.handleMultipleCalendars(
        selectedCalendars,
        classificationCalendar,
        nameCalendar,
        classVisibility,
      ...
      return reply
        .status(400)
        .send({ error: "More than one calendar must be selected" });
    }
  ...
  async logout(req, reply) {
    ...
    return reply.status(200).view("logout.ejs", {
      title: "disconnect",
      error: "You are now disconnected",
      token: null,
    });
  }
  ...
  async getUserCalendars(req, reply) {
    console.log("Get user calendars request received");
    ...
    try {
      ...
      if (!user || !user.hasCalendar(calendarId)) {
        return reply.status(404).send({
          error: user
            ? "You don't have the right to update this calendar"
            : "User not found",
        });
      }
      ...
      calendar.syncExpressionCron =
        calendarUpdated.syncExpressionCron != "0"
          ? calendarUpdated.syncExpressionCron
          : calendar.syncExpressionCron;
      ...
      if (
        calendar.url != "" &&
        Task.validate(calendar.syncExpressionCron) === false
      ) {
  ...
  async createUserCalendar(req, reply) {
    ...
    try {
      ...
      return reply
        .status(200)
        .send({ message: "Calendar created successfully", calendar: calendar });
    } catch (error) {
      console.log(error);
      return reply.status(500).send({ error: "An unexpected error occurred" });
    }
  ...
  async importUserCalendar(req, reply) {
    ...
    const user = (await User.getBy("username", id))[0];

    if (!user || !user.haveAccessToCalendar(calendarId)) {
    ...
    const uploadCalendar = new Calendar({ source: filePath });
    await uploadCalendar.parseEvents();

    const conflicts = [];
    for (const importedEvent of uploadCalendar.events) {
      const existingEvent = await Event.getById(importedEvent.id);
      if (existingEvent) {
        conflicts.push({
    ...
    if (conflicts.length == 0) {
      ...
      reply
        .status(200)
        .send({ message: `Calendar imported successfully without conflicts` });
    } else {
      console.log(
        `File uploaded successfully with ${conflicts.length} conflicts`
      );
      reply.status(409).send({ conflicts });
    }
  }

  async updateUserCalendarEvents(req, reply) {
    console.log("Update event calendar request received");
    ...
    if (!user.haveAccessToCalendar(calendarId)) {
    ...
    for (const event of events) {
      let eventToUpdate = await Event.getById(event.id);
      ...
      eventToUpdate.start = new Date(event.start).toISOString();
      eventToUpdate.end = new Date(event.end).toISOString();
      eventToUpdate.description =
        event.description || eventToUpdate.description;
      eventToUpdate.location = event.location || eventToUpdate.location;
      eventToUpdate.transp = event.transp || eventToUpdate.transp;
      eventToUpdate.rRule = event.recurrence || eventToUpdate.rRule;

      await eventToUpdate.persist();
    }
  ...
  async subscribeUserCalendar(req, reply) {
    ...
    return reply
      .status(200)
      .send({ message: "Calendar subscribed successfully" });
  }
  ...
  async unsubscribeUserCalendar(req, reply) {
    ...
    return reply
      .status(200)
      .send({ message: "Calendar unsubscribed successfully" });
  }
}
