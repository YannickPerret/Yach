...
export default function SideBarUserItem({calendar, onCalendarClick, onEditClick}) {
    return (
        <div key={calendar.id} onClick={() => onCalendarClick(calendar.id)}>
            ...
            {calendar.name}
            <button data-id={calendar.id} className="sidebar-userAllCalendars-editBtn" onClick={() => onEditClick(calendar.id)} >Editer</button>
        </div>
    )
}